...
- 固定在屏幕一处空位，**只在桌面第 1 页显示**；离开第 1 页、切走应用、点过桌面图标都会自动让位。
- 图墨（Tomo）与 ELauncher 两个桌面都支持；默认桌面在两个之间切换后**立刻生效**，不用重启无障碍服务。
...
- JDK 17 —— **这是唯一验证过的版本**。脚本会探测更高版本，但"更高"没有实测过
  （外部审查用 JDK 21 + build-tools 34 时 d8 直接抛 `NullPointerException`）。
- Android SDK：`build-tools;33.0.2` + `platforms;android-33`。脚本在找不到这一对时会退到
  次选版本（34/32/31/30）并**打印一行警告** —— 若你把构建结果用于发布，请以首选组合重跑。
  也可以用 `BT_VERSION=33.0.2` / `PLATFORM_VERSION=33` 显式指定。
- Python 3（构建脚本用它做打包与清单生成）
- **Windows + Git Bash**（脚本依赖 `cygpath` 与 `.exe`/`.bat` 形式的 SDK 工具，
  Linux / macOS 不能直接照跑）
...
- 「离开第 1 页就收起」靠桌面投递的翻页事件判定，目前覆盖**图墨（Tomo）与 ELauncher**
  两个桌面。换用别的桌面、或桌面改版导致事件形态变化时，判据认不出就不动作（卡片照常
  显示），可在设置页点「重新同步卡片显示」手动恢复。
