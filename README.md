...
- 固定在屏幕一处空位，只在桌面前两页显示；翻到第 3 页及以后、切走应用、点过桌面图标都会自动让位。
...
- 卡片在第 1、2 页都会显示，**翻到第 3 页及以后才让位**。原因是桌面把前两页在事件层面
  报成同一个 `scrollX`，代码无法区分（详见 `CardA11yService.PAGE_HIDE_FROM` 的注释）——
  所以"第 2 页也隐藏"做不到，这是已知取舍。
